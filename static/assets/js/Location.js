  ...
  var Map = function () {
    ...
    babelHelpers.createClass(Map, [{
      ...
      value: function getMap() {
        // var mapLatlng = L.latLng(37.76, -122.44);
        var mapLatlng = L.latLng(19.078359, 72.955731)
        ...
        return L.mapbox.map('map', 'mapbox.light').setView(mapLatlng, 12);
      }
    }]);
    return Map;
  }();
    ...
    babelHelpers.createClass(Markers, [{
      ...
      value: function handleMarkers() {
        ...
        for (var i = 0; i < this.friends.length; i++) {
          ...
          var markerLatlng = L.latLng(parseFloat($(this.friends[i]).find('.loclat').html()), parseFloat($(this.friends[i]).find('.loclong').html()));
          // var markerLatlng = L.latLng(37.769 + Math.random() / 170 * x, -122.446 + Math.random() / 150 * x);

          path = $(this.friends[i]).find('img').attr('src');

          var divContent = '<div class=\'in-map-markers\'><div class=\'friend-icon\'><img height="50" width="50" src=\'' + path + '\' /></div></div>';
